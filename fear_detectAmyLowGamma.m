function fear_detectAmyLowGamma(basename,varargin)
%%
...
amyLowGamma.timestamps=[];
amyLowGamma.sh={};
amyLowGamma.peaks.timestamps=[];
amyLowGamma.peaks.power=[];
amyLowGamma.peaks.sh=[];
while size(tGamma,1)>0
    ...
    amyLowGamma.timestamps(end+1,:)=target;
    amyLowGamma.sh{end+1}=tempSh;
    amyLowGamma.peaks.timestamps(end+1)=peakTime;
    amyLowGamma.peaks.power(end+1)=peakVal;
    amyLowGamma.peaks.sh(end+1)=peakSh;
...
fprintf('%s After marge across ch: %d events\n',...
    datestr(now),size(amyLowGamma.timestamps,1))

ngIdx=find(diff(amyLowGamma.timestamps,1,2)>param.maxDuration);
amyLowGamma.timestamps(ngIdx,:)=[];
amyLowGamma.sh(ngIdx)=[];
amyLowGamma.peaks.timestamps(ngIdx)=[];
amyLowGamma.peaks.power(ngIdx)=[];
amyLowGamma.peaks.sh(ngIdx)=[];
fprintf('                     %d events passed duration criterion\n',...
    size(amyLowGamma.timestamps,1))

%%
[~,order]=sort(amyLowGamma.timestamps(:,1));

amyLowGamma.timestamps=amyLowGamma.timestamps(order,:);
amyLowGamma.sh=amyLowGamma.sh(order);
amyLowGamma.peaks.timestamps=amyLowGamma.peaks.timestamps(order)';
amyLowGamma.peaks.power=amyLowGamma.peaks.power(order)';
amyLowGamma.peaks.sh=amyLowGamma.peaks.sh(order)';

%%
behType=zeros(size(amyLowGamma.peaks.timestamps));
for n=1:3
    target=beh(beh(:,3)==n*2-1,1:2);
    behType(any(amyLowGamma.peaks.timestamps>target(:,1)'&amyLowGamma.peaks.timestamps<=target(:,2)',2))=n;
end
amyLowGamma.state=behType;
amyLowGamma.stateName={'Wake','NREM','REM'};
amyLowGamma.region=region;
%%
amyLowGamma.generator=mfilename;
amyLowGamma.generatedate=datestr(now,'yyyy-mm-dd');
amyLowGamma.param=param;
%%
fprintf('%s Saving results\n',datestr(now))
save([basicMetaData.Basename '.amyLowGamma.events.mat'],'amyLowGamma','-v7.3')
