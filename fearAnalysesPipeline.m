function fearAnalysesPipeline(rootDir, sessionName,chName,detectionintervals,varargin)
...
%video
if reAnalyses || ~exist([basicMetaData.Basename '.videoFrames.events.mat'],'file')
    detectTTLpulses(basicMetaData,...
        ...
        'detectionintervals',basicMetaData.detectionintervals.ttl,...
        'filetype','dat',...
        'evtFileName',[basicMetaData.BaseDir '/lfp/' basicMetaData.SessionName '.vtr.evt'],...
        'saveFileName',[basicMetaData.Basename '.videoFrames.events.mat'],...
        'varName','videoFrames');
else
    if verbose
        disp('    Video frame TTL detection is skipped : already done')
    end
end

%clock
if reAnalyses || ~exist([basicMetaData.Basename '.clocks.events.mat'],'file')
    detectTTLpulses(basicMetaData,...
        ...
        'detectionintervals',basicMetaData.detectionintervals.ttl,...
        'filetype','dat',...
        'evtFileName',[basicMetaData.BaseDir '/lfp/' basicMetaData.SessionName '.clk.evt'],...
        'saveFileName',[basicMetaData.Basename '.clocks.events.mat'],...
        'varName','clocks');
else
    if verbose
        disp('    Clock TTL detection is skipped : already done')
    end
end



%% LED blink during behavior sessions
if ~exist(fullfile(basicMetaData.BaseDir,'videoDetection'),'dir')
    mkdir(fullfile(basicMetaData.BaseDir,'videoDetection'))
end
for idx=1:length(basicMetaData.chamber)

    
    saveMatFilename=fullfile(basicMetaData.BaseDir,'videoDetection',[sessionName '_' basicMetaData.chamber(idx).name '_ledBlink.mat']);
    if reAnalyses || ~exist(saveMatFilename,'file')
        
        
        targetVideoFile=basicMetaData.video(strcmpi({basicMetaData.video.filename},basicMetaData.chamber(idx).videofile));
        if isempty(targetVideoFile)
            warning('video filename is wrong for LED detection: skipped for %s',basicMetaData.chamber(idx).name)
            continue
        end
        ledRange=targetVideoFile(1).ledRange;
        ledReadFromVideo2(basicMetaData.chamber(idx).videofile,ledRange,...
            basicMetaData.chamber(idx).detectionintervals(1),basicMetaData.chamber(idx).detectionintervals(2),...
            basicMetaData.chamber(idx).name,saveMatFilename)
    else
        if verbose
            fprintf('    led blink detection of %s is skipped: already done\n',basicMetaData.chamber(idx).name)
        end
    end
    
end
%% detect clock LED for entire video
if reAnalyses || ~exist(fullfile(basicMetaData.BaseDir,'videoDetection',[sessionName '_' 'clockLED.mat']),'file');
    saveMatFilename=fullfile(basicMetaData.BaseDir,'videoDetection',[sessionName '_' 'clockLED.mat']);

    ledReadFromVideo2(basicMetaData.video.filename,basicMetaData.video.ledRange(1,:),...
            [],[],...
            '',saveMatFilename);
end
...
%     candCh=candCh(ismember(basicMetaData.Ch.names(candCh),{'CeA','LA','BLA'}));
...
if redoSlowWave || reAnalyses || ~exist([basicMetaData.Basename '.pfcSlowWave.events.mat'],'file')
    nCh=cellfun(@length,basicMetaData.chMap);
    chList=[basicMetaData.chMap{cellfun(@(x) all(x>64 & x<=128),basicMetaData.chMap)& nCh>=8}];
