function fear_detectAmyHighGamma(basename,varargin)
%%
...
amyHighGamma.timestamps=[];
amyHighGamma.sh={};
amyHighGamma.peaks.timestamps=[];
amyHighGamma.peaks.power=[];
amyHighGamma.peaks.sh=[];
while size(tGamma,1)>0
    ...
    amyHighGamma.timestamps(end+1,:)=target;
    amyHighGamma.sh{end+1}=tempSh;
    amyHighGamma.peaks.timestamps(end+1)=peakTime;
    amyHighGamma.peaks.power(end+1)=peakVal;
    amyHighGamma.peaks.sh(end+1)=peakSh;
...
fprintf('%s After marge across ch: %d events\n',...
    datestr(now),size(amyHighGamma.timestamps,1))

ngIdx=find(diff(amyHighGamma.timestamps,1,2)>param.maxDuration);
amyHighGamma.timestamps(ngIdx,:)=[];
amyHighGamma.sh(ngIdx)=[];
amyHighGamma.peaks.timestamps(ngIdx)=[];
amyHighGamma.peaks.power(ngIdx)=[];
amyHighGamma.peaks.sh(ngIdx)=[];
fprintf('                     %d events passed duration criterion\n',...
    size(amyHighGamma.timestamps,1))

%%
[~,order]=sort(amyHighGamma.timestamps(:,1));

amyHighGamma.timestamps=amyHighGamma.timestamps(order,:);
amyHighGamma.sh=amyHighGamma.sh(order);
amyHighGamma.peaks.timestamps=amyHighGamma.peaks.timestamps(order)';
amyHighGamma.peaks.power=amyHighGamma.peaks.power(order)';
amyHighGamma.peaks.sh=amyHighGamma.peaks.sh(order)';

%%
behType=zeros(size(amyHighGamma.peaks.timestamps));
for n=1:3
    target=beh(beh(:,3)==n*2-1,1:2);
    behType(any(amyHighGamma.peaks.timestamps>target(:,1)'&amyHighGamma.peaks.timestamps<=target(:,2)',2))=n;
end
amyHighGamma.state=behType;
amyHighGamma.stateName={'Wake','NREM','REM'};
amyHighGamma.region=region;
%%
amyHighGamma.generator=mfilename;
amyHighGamma.generatedate=datestr(now,'yyyy-mm-dd');
amyHighGamma.param=param;
%%
fprintf('%s Saving results\n',datestr(now))
save([basicMetaData.Basename '.amyHighGamma.events.mat'],'amyHighGamma','-v7.3')
