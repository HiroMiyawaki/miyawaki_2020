function fear_gammaTrigIcaCoactStrength(basename,varargin)
%%
...
load([basicMetaData.Basename '.pfcHighGamma.events.mat'])
...
pfcGamma(1).param.freqRange

for tRangeType=1:2
    clear res
    if tRangeType==1
        tRange=sessions.homecage(3,:);
        typeMax=2+length(pfcGamma);
        behIdx=3; %nrem
        fileName='gammaTrigIcaCoact-postNREM.mat';
        varName='gammaTrigCoact';
        ...
        typeMax=2+length(pfcGamma);
        behIdx=1; %awake
        fileName='gammaTrigIcaCoact-cueRet.mat';
        varName='gammaTrigCoact';
    ...
    for trigType=1:typeMax
        if trigType==1
                ...
                gmParam=pfcHighGamma.param;
        elseif trigType==2
                trigT=pfcLowGamma.peaks.timestamps;
                gmParam=pfcLowGamma.param;
        else
                trigT=pfcGamma(trigType-2).peaks.timestamps;
                gmParam=pfcGamma(trigType-2).param;
                gmParam.freqRange=pfcGamma(trigType-2).freqRange;
